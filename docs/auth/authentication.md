...
- **MFA required for administrative accounts** (AUTH-116) is **partially** achieved: the mechanism exists (an active `ADMIN` role assignment routes login into the standard MFA-challenge path exactly like any other `mfaEnabled` user, and issues an `admin`-typed session with a stricter timeout policy), but there is no user-facing "grant the ADMIN role" flow anywhere in this codebase yet to hook a hard "you may not have admin without MFA" gate into — role assignment is currently only reachable via seed/migration/direct DB action. This is named as an open item (§7) rather than silently claimed as complete.
...
- **SEC-032 brute-force protection**: 10 consecutive failed attempts locks the account (`status='locked'`, `locked_until` 15 minutes out) and raises `LOGIN_BRUTE_FORCE`/`ACCOUNT_LOCKED` security events. The lock **self-clears** on the next login attempt after `locked_until` passes — a documented, deliberately minimal "defined recovery path" per the Phase 4 instruction to establish only the necessary foundation; there is no admin-initiated unlock endpoint yet (§7).
- **SEC-031/API-017 rate limiting**: an in-memory sliding-window limiter (`apps/api/src/modules/auth/rate-limit.ts`) in front of `/login`, `/password/forgot`, `/password/reset`, and `/mfa/verify`, independent of the account-lockout mechanism. **Known limitation**: in-memory state does not share across multiple backend instances — adequate for CON-02's current single-VPS-process production target, not for a horizontally-scaled deployment. Written behind a `RateLimiter` interface specifically so it can be swapped for the `Cache` port (`docs/architecture/backend.md` §5 — KV/Durable Objects for MVP, Redis for production) without touching callers, the same BE-008 adapter pattern used everywhere else in this codebase.
...
- **AUTH-116 admin-MFA enforcement is partial** — see §3. No hard gate blocks an admin account from operating without MFA, because there is no admin-role-granting flow yet to gate.
- **SEC-032 recovery path is time-based only** — no admin-assisted unlock endpoint exists yet; a locked account must wait out the 15-minute window.
