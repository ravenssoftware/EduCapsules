...
- `test_ids` is a `;`-separated list of test identifiers (file path + test name, e.g. `apps/api/tests/authz.spec.ts::denies cross-tenant read`).
...
1. Parse the SRS's requirement-ID inventory (extractable from `docs/srs/EduCapsules_Master_SRS_v1.1.md` — every `**XXX-nnn**` bolded ID in a requirements table).
...
| End-to-end | The 22 SRS use cases (§46) driven through the real client | Every release |
...
| Accessibility | Automated axe-class checks every screen; manual keyboard/screen-reader on core flows | Automated every PR; manual before release |
