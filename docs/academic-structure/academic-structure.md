...
| Enrollment | request / activate / withdraw | `MANAGE_STUDENTS` | `VIEW_STUDENTS` (per-course); self-access for a Student's own list (see §7) |
...
### 4.6 Enrollment state machine (§45.3) — REQUESTED → ACTIVE, added in the Phase 6 review-fix pass

SRS §45.3's "Other lifecycles" table (the *only* place in the entire SRS that names Enrollment's states) reads: `Enrollment REQUESTED → ACTIVE → (WITHDRAWN | COMPLETED | TRANSFERRED)`. The initial Phase 6 implementation skipped straight to `active` on creation; the Project Owner's Phase 6 review corrected this — see §10.3 for the full decision record. As implemented now:

- `POST /courses/:id/enrollments` creates the row in **`requested`** status (`createEnrollment`, gated by `MANAGE_STUDENTS`, the only authority the SRS names anywhere for this entity).
- `POST /enrollments/:id/activate` transitions `requested → active` (`activateEnrollment`, gated by the **same** `MANAGE_STUDENTS` authority — see §10.3 for why no distinct approver role was invented).
- `POST /enrollments/:id/withdraw` remains `active → withdrawn` only, exactly as before; withdrawing a still-`requested` row is rejected (`409`) — the SRS's own edge list does not offer a `REQUESTED → WITHDRAWN` transition, and none is invented.
- `completed` and `transferred` remain unreached by any code path — see §5.

## 5. Known limitations (reported, not silently absorbed)

- **PER-002's Teacher-delegation clause — DEFERRED, Project-Owner decision.** "Authorized Admins, and Teachers where the Organization delegates it, shall create and manage AcademicPeriods." No Organization-settings field or mechanism exists anywhere in the SRS or schema to determine when an Organization "delegates" this to Teachers. The Project Owner reviewed this and explicitly decided to **defer** building the delegation mechanism and **keep** the Admin-only implementation for Phase 6's scope (see §10.3) — this is a settled disposition, not an open question, pending a future explicit design decision.
- **PER-004's concurrent-periods opt-in — DEFERRED, Project-Owner decision.** The non-overlap rule is enforced unconditionally for every Organization — the "unless the Organization explicitly supports concurrent periods" exception has no settings field to build against. The Project Owner reviewed this and explicitly decided to **keep** unconditional non-overlap enforcement and **not** build a new Organization-settings field merely to support the undefined opt-in (see §10.3) — settled, not open.
- **CRS-007's Submission-existence gate — acceptable, belongs to a later phase.** "A Course shall not be returned to draft once a Submission exists against any of its Activities." The `Submission` entity does not exist until Phase 12 (Question Bank/Activities/Grading phases). The DRAFT↔PUBLISHED↔ARCHIVED state machine itself is implemented and tested correctly; the specific gate blocking unpublish once real student work exists against the Course is a documented gap for Phase 12 to close, not silently omitted or faked with a placeholder check.
- **CLS-008/GRP-003's content-targeting behavior — acceptable, belongs to a later phase.** CourseAudience add/remove/list — the *mechanism* that records which Classroom/Group a Course is targeted to — is implemented and tested. The actual consequence (a Student in that Classroom/Group being able to *see* the Course, its Cycles/Topics, or its content) is not implemented: `apps/api/src/modules/authz/service.ts`'s `resolveAuthoritySources()` was deliberately **not** extended in Phase 6 to resolve Student/Parent visibility across the content tree via CourseAudience membership — that is Activities/Content-phase work (explicitly out of Phase 6 scope per the Phase 0 roadmap), and extending the core authz pipeline for it was judged to be scope creep beyond "academic structure." A Student or Parent currently has no authorization path to read Subject/Course/Cycle/Topic records at all (they see Classroom/Group only, as Phase 5 already supported).
- **Enrollment's requester/approver split — reported ambiguity, not invented (see §10.3).** SRS §45.3 names the REQUESTED and ACTIVE states but describes no use case, no actor and no approval mechanism for the transition between them anywhere in the document (confirmed by an exhaustive search, including Appendix A's requirement-prefix registry, which has no `ENR` prefix at all — Enrollment is not an owned requirement area). Per the Project Owner's explicit instruction, this was **not** invented: both `createEnrollment` (REQUESTED) and `activateEnrollment` (REQUESTED→ACTIVE) are gated by the one authority the SRS's broader model supports for this entity (`MANAGE_STUDENTS`, i.e. Teacher/Admin), with no distinct requester-vs-approver actor built. Whether a genuinely separate approver role/workflow is wanted is a decision only the Project Owner can make.
- **Enrollment's COMPLETED/TRANSFERRED states remain unreached.** `completed` (course/period completion) depends on progress-tracking machinery that doesn't exist yet (§17/PRG, a later phase). `transferred` is produced by the *move-student* workflow's own Enrollment cascade (SRS §46.2's UC: "enrolments driven by the source classroom are marked TRANSFERRED... enrolments driven by the destination classroom are created") — this is a materially larger feature (deriving Enrollment rows automatically from CourseAudience targeting, which this module does not do; every Enrollment today is created by an explicit `POST .../enrollments` call, never auto-derived) that was judged out of scope for this defect-correction pass. `service.ts`'s `moveStudent()` still only moves the Classroom Membership, exactly as before; it does not touch Enrollment rows.
...
`apps/api/src/routes/academic-structure.test.ts` — 68 tests (57 from the initial Phase 6 commit + 11 added by the review-fix pass, §10), full HTTP-surface integration tests against a real in-memory SQLite database migrated with the real generated migrations, through the real routes, exercising the actual `authorize()` pipeline end-to-end (never mocked). Covers: CRUD happy/invalid paths for every entity, PER-003/PER-004 business rules, the move-student operation (including old-row preservation), polymorphic reference validation, duplicate handling (Subject code, Cycle/Topic sequence numbers, duplicate co-teacher, duplicate audience target, duplicate Enrollment request), Course lifecycle transitions and their invalid-transition rejections, co-teacher grant/revoke/ownership-restriction, cross-organization isolation, forged client fields (`ownerTeacherId`, `organizationId`, `academicPeriodId`), IDOR attempts against every entity type, the Enrollment REQUESTED→ACTIVE→WITHDRAWN workflow, and SEC-009 denial-semantics assertions (object-identified denials report 404, not 403, with response **body** comparisons, not just status codes — see §10.1).
...
`packages/db/src/schema/db-behavior-scenarios.ts` — 2 new scenarios (`academic_periods.status`, `enrollments.status` CHECK constraints), run automatically against both real engines by `sqlite/db-behavior.test.ts` and `postgres/db-behavior.test.ts` (§10.3).

Full regression after the review-fix pass: 272/272 tests pass — 257 from the initial Phase 6 commit (200 pre-Phase-6 + 57 initial `academic-structure.test.ts`) + 11 new tests in `academic-structure.test.ts` (§10.1/§10.2/§10.4/§10.5/§10.6, plus the Enrollment-workflow tests from §4.6) + 4 from the 2 new `db-behavior-scenarios.ts` entries running once per dialect (§10.3) — verified against **both** real SQLite and real PostgreSQL, plus `typecheck`, `lint`, `format`, and the Playwright e2e smoke suite, all clean.

## 10. Phase 6 review-fix pass (Project Owner review, corrections applied)

The Project Owner's post-completion review (against this same SRS text) found four defects and flagged three items for an explicit decision. All four defects were fixed and regression-tested; all three decisions were resolved by the Project Owner (recorded in §10.3). This section is the permanent record of that pass — the fixes themselves are described in-place above (§3's SEC-009 note, §4.5's audit-action note is below, §4.6's Enrollment workflow) and are not repeated in full here.

### 10.1 Defect 1 — SEC-009 message-text leak (fixed)

Every "create a child under an existing parent" action (Group, Cycle, Topic, Membership, CourseAudience, co-teacher add/remove, Enrollment) checked the parent's existence *before* calling `authorize()`, using a distinct custom message (`"Course not found."`) — while `authorize()`'s own denial for the identical case used a generic message (`"Not found."`). Both were 404, but the **response body text differed**, letting a caller distinguish "this id doesn't exist" from "this id exists but you can't see it" by reading the message rather than the status code — exactly what SEC-009 forbids. Confirmed empirically before fixing (a throwaway script, not committed) and fixed by using the same generic `notFound()` (no custom message) in every affected call site in `service.ts`. Regression tests: three new tests directly comparing response bodies (not just status codes) for Group/Cycle/Enrollment creation against a real-but-unauthorized vs. a genuinely nonexistent parent id.

### 10.2 Defect 2 — audit action string typo (fixed)

`transitionCourseStatus`'s audit-action derivation, `` `COURSE_${action.toUpperCase()}D` ``, produced `COURSE_PUBLISHD` and `COURSE_UNPUBLISHD` (missing the trailing `E`) — it only happened to be correct for `archive`→`COURSE_ARCHIVED` by coincidence. Fixed by replacing the derived string with an explicit `auditAction` field on each of the three call sites (`publishCourse`/`unpublishCourse`/`archiveCourse`), so the canonical action string is spelled out once, never templated from the verb. Regression test: creates a Course, runs publish → unpublish → archive, and asserts the exact `audit_log.action` values are `COURSE_PUBLISHED`/`COURSE_UNPUBLISHED`/`COURSE_ARCHIVED`, and explicitly asserts the old misspelled forms never appear again.

### 10.3 Defect 3 — missing dual-dialect CHECK-constraint regression coverage (fixed)

Both new CHECK constraints (§8) were verified manually during the original Phase 6 implementation but that verification wasn't preserved as regression coverage — `packages/db/src/schema/db-behavior-scenarios.ts` (the shared suite every other CHECK constraint in this schema already has an entry in, run automatically against both real engines) had no entry for either. Fixed by adding `"applies a CHECK constraint on academic_periods.status (PER-001, Phase 6)"` and `"applies a CHECK constraint on enrollments.status (§45.3 state machine, Phase 6)"` scenarios, following the exact existing pattern (insert a valid control row, then assert an invalid value is rejected). Confirmed running and passing against both `sqlite/db-behavior.test.ts` and `postgres/db-behavior.test.ts`.

### 10.4 Defect 4 — overly broad catch blocks in list endpoints (fixed)

`listClassrooms`/`listSubjects`/`listCourses` each wrapped a per-candidate `authorize()` call in a bare `catch {}`, silently omitting the row on *any* thrown error — not only the expected authorization denial. A genuine bug or repository failure for one row would have silently vanished it from the list rather than surfacing. Fixed by narrowing each catch to `if (!(err instanceof AuthzDomainError)) throw err;` — only the expected denial type is swallowed; anything else propagates. Regression test: a Proxy-wrapped `authzRepository` is used to force a plain `Error` (not `AuthzDomainError`) for one specific classroom id, and the test asserts `service.listClassrooms(...)` rejects with that error rather than returning a shorter list.

### 10.5 Minor issue 5 — invalid/cross-org user references (fixed, per SRS Table 40.2)

An invalid or cross-organization `studentUserId` (Enrollment) or `userId` (Membership) previously fell through to a raw database composite-FK violation, caught only by the generic error handler as an opaque `500 Internal Server Error` — safe (no internal detail leaked, confirmed by reading `middleware/error-handler.ts`) but not correct: SRS Table 40.2 ("Canonical condition → response mapping") maps an invalid/malformed client reference to a validation-class response, never `500` (`500` is reserved for "unhandled server fault"). Fixed by adding `AcademicStructureRepository.userExists(organizationId, userId)` and calling it in `addMembership`/`createEnrollment` before the insert, raising a `422 validation_failed` (this module's existing convention for input validation) instead. Regression test: requests an Enrollment for a `studentUserId` that doesn't exist and asserts `422`.

### 10.6 Test-title correction (fixed)

The test titled "denies a different Teacher from creating a Course under a Subject they have no authority over" actually exercised a nonexistent-subject-id path using the *same* Teacher who created the Subject — it never tested an unrelated Teacher at all (Subject has no owner field, so `teacher2`, the fixture's other Teacher, holds independent ORG-wide `MANAGE_COURSE` authority and would incorrectly pass). Split into two accurately-titled tests: one for the nonexistent-subject-id 404 path (kept, retitled), and a new one using the fixture's `createUnrelatedTeacher()` helper (a Teacher whose role assignment is CLASSROOM-scoped, which never covers a SUBJECT-scope target) — this genuinely exercises "an unrelated Teacher is denied."

### 10.7 Project Owner decisions (recorded verbatim in disposition)

- **PER-002 (Teacher delegation): DEFER.** Keep Admin-only. No delegation mechanism, configuration surface, scope or revocation model is defined anywhere in the SRS — inventing one was explicitly declined. Documented as a deferred/open requirement in §5, not implemented.
- **PER-004 (concurrent AcademicPeriods): KEEP unconditional non-overlap.** No new Organization-settings field was added merely to support the undefined opt-in. Documented as deferred in §5; concurrent periods are **not** supported, and this is not silently overclaimed anywhere.
- **Enrollment REQUESTED workflow: IMPLEMENT the state machine now.** §4.6/§5 above. The states and the one transition edge the SRS actually specifies (`REQUESTED → ACTIVE`) are implemented; the *actor* question (is there a distinct approver, separate from whoever requested?) was investigated exhaustively (full-document search, Appendix A prefix-registry check confirming no `ENR` requirement area exists, UC list review) and found genuinely undefined — per the Project Owner's own explicit instruction, this was reported rather than invented, and both steps use the one authority (`MANAGE_STUDENTS`) the SRS's broader model already establishes for this entity.

No contradiction in the SRS itself was found while implementing the REQUESTED workflow — only an absence of specification for the actor question, which is a gap to fill by decision, not a contradiction to resolve by textual correction. The Master SRS v1.0 was not modified.
