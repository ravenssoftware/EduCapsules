...
**Resolved in Phase 4** (`docs/auth/authentication.md`): an opaque, server-side, hash-stored session token (not a JWT) — since AUTH-105/109's individual server-side revocation requirement means a JWT would need its own revocation list anyway; TOTP-only MFA per AUTH-116/126, implemented; session-timeout values set to conservative points inside Table 35.3's own indicative bands, explicitly still provisional pending D-14. Password hashing (Argon2id) and the TOTP HMAC both deliberately avoid Node-native bindings and use pure WebAssembly / Web Crypto instead, specifically so this stage-1 credential mechanics stays portable to the CON-01 Workers MVP target once that deployment is actually wired up — see that document §2 for the full reasoning.

**Resolved in Phase 5** (`docs/authz/authorization.md`): the §7.1 pipeline itself, implemented as `apps/api/src/modules/authz/`'s `authorize()` — role/ownership/membership/ParentLink/AssistantAssignment resolution, §7.3 scope containment across the cohort and content trees, §26's permission catalogue as seeded data, and the full Assistant delegation (§21) and ParentLink (§22) lifecycles including SEC-006/SEC-014's non-amplification and live re-verification. §23/§26.3-4's platform-governance sub-roles remain out of scope — the Phase 0 roadmap's own later "Administration" phase, not Phase 5's.
