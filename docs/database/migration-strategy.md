...
| 0005 | `0005_*.sql` | Phase 5: adds `assistant_assignments`, `assistant_assignment_scopes`, `assistant_assignment_permissions`, and `parent_links`. Pure new-table creation (no existing-table rebuild), generated cleanly on both dialects with no hand-fixing needed, applied to real SQLite and PostgreSQL instances. |
| 0006 | `0006_*.sql` | Phase 6: adds `academic_periods_status_check` (`planned`\|`active`\|`closed`) and `enrollments_status_check` (`requested`\|`active`\|`withdrawn`\|`completed`\|`transferred`) — see `docs/academic-structure/academic-structure.md` §8. On SQLite this required the same full-table-rebuild pattern as migration 0003 (adding a CHECK constraint isn't a simple `ALTER TABLE`), but since no new columns were involved this time the generated `INSERT INTO __new_x ... SELECT` was correct as generated — no hand-fixing needed, unlike 0003. On PostgreSQL, a plain `ALTER TABLE ... ADD CONSTRAINT`. Both verified against real, freshly created SQLite and PostgreSQL instances, including that the constraint actually rejects an out-of-enumeration value. |
